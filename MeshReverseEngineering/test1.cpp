...
int main() {
            ...
            for (int counter = 0; counter < counters.size(); ++counter) {
                ...
                if (rc64 < 0) {
                    ...
                    cout << "Configuring socket" << socket << "-CHA" << cha << " by writing 0x" << std::hex << msr_val
                         << " to core " << std::dec << core << ", offset 0x" << std::hex << msr_num << std::dec
                         << "\n";
                }
            }
        }
    ...
    /// Flush the data from the cache in case it is in the cache somehow (might be futile here but just wanted to make
    /// sure).
